...
def cargar_datos_csv(nombre_csv):
    if os.path.exists(nombre_csv):
        with open(nombre_csv, mode='r') as archivo_csv:
            ...
            return {fila[0]: fila[1:] for fila in lector_csv}
    return {}
...
def click(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        # Cambiar color por par de clics
        ...
        # Mostrar las coordenadas en el lugar donde se ha hecho clic
        cv2.putText(imagen_actual, f"({x}, {y})", (x + 10, y + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
        
        # Mostrar la imagen actualizada con los puntos
...
def dibujar_recuadros(imagen, puntos, etiquetas_c, colores):
    ...
    for i in range(len(puntos)):
        ...
        (x1, y1) = puntos[i][0]  # Primer punto (x1, y1)
        (x2, y2) = puntos[i][1]  # Segundo punto (x2, y2)
        
        # Obtener la etiqueta c asociada al recuadro
        etiqueta_c = etiquetas_c[i]  # Obtener la etiqueta c para este par de puntos
        color = colores[indice_color]  # Seleccionamos un color para el recuadro
        
        # Dibujar el recuadro en la imagen
        cv2.rectangle(imagen, (x1, y1), (x2, y2), color, 2)
        
        # Escribir la etiqueta c sobre el recuadro
...
def actualizar_imagen():
    ...
    puntos = []  # Asegurarse de que 'puntos' esté vacío al comienzo
    etiquetas_c = []  # Para almacenar las etiquetas 'c'
    
    # Si hay datos existentes, dibujarlos
...
# Crear las variables de los checkboxes después de la inicialización de root
checkbox_vars = [BooleanVar(value=True) for _ in range(4)]
